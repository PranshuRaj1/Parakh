...
export interface LLMProvider {
  reviewDiff(fileName: string, diff: string, activeRules: Rule[], context?: LLMRequestContext, referenceFileContent?: string): Promise<ReviewResult>;
  reviewIncrementalDiff(fileName: string, diff: string, activeRules: Rule[], priorFindings: Finding[], context?: LLMRequestContext, referenceFileContent?: string): Promise<IncrementalReviewResult>;
...
export class LLMClient {
  ...
  reviewDiff(fileName: string, diff: string, rules: Rule[], signal?: AbortSignal, referenceFileContent?: string): Promise<ReviewResult> {
    return this.route('review', this.providers, async (provider, context) => {
      const result = await provider.reviewDiff(fileName, diff, rules, context, referenceFileContent);
  ...
  reviewIncrementalDiff(fileName: string, diff: string, rules: Rule[], findings: Finding[], signal?: AbortSignal, referenceFileContent?: string): Promise<IncrementalReviewResult> {
    return this.route('incremental_review', this.providers, async (provider, context) => {
      const result = await provider.reviewIncrementalDiff(fileName, diff, rules, findings, context, referenceFileContent);
      if (!Array.isArray(result.genericFindings) || !Array.isArray(result.ruleFindings)) {
